...
public class Exam {
    ...
    public void setAimPic() {
        ...
        for (int i = 0; i < this.array.length; i++) {
            ...
            /*
             * for(int j = 0; j < this.poiseGrayArray.length; j++){
             * if(this.grayArray[i]==j){ this.aimPic[i] =
        ...
        for (int i = 0; i < this.height; i++) {
            for (int j = 1; j <= this.width; j++) {
                ...
                if (i == 0 || i == this.height - 1 || j == 1 || j == this.width) {
                    ...
                    // this.removedNoise[i*this.width+j-1] = (255 << 24) |
                    // (this.r[i*this.width+j-1] << 16) |
                    // (this.g[i*this.width+j-1] << 8) |
                    // this.b[i*this.width+j-1];
                } else {
    ...
    public static void main(String[] argv) throws Exception { // Pic类的测试函数
        // 已经全部通过了 ioi
        Exam pic = new Exam("C:\\Users\\Gepeng18\\Desktop\\1.jpg");// 加载一张图片
        System.out.println(pic.height + "***" + pic.width);// 输出图片的高宽像素

        // 均值去噪
        BufferedImage image1 = new BufferedImage(pic.getWidth(), pic.getHeight(), BufferedImage.TYPE_3BYTE_BGR);
        image1.setRGB(0, 0, pic.getWidth(), pic.getHeight(), pic.getRemovedNoise1(), 0, pic.getWidth());
        ImageIO.write(image1, "jpg", new File("C:\\Users\\Gepeng18\\Desktop\\2.jpg"));

        // 中值去噪
        BufferedImage image2 = new BufferedImage(pic.getWidth(), pic.getHeight(), BufferedImage.TYPE_3BYTE_BGR);
        image2.setRGB(0, 0, pic.getWidth(), pic.getHeight(), pic.getRemovedNoise2(), 0, pic.getWidth());
        ImageIO.write(image2, "jpg", new File("C:\\Users\\Gepeng18\\Desktop\\3.jpg"));
